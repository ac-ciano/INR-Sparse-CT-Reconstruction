"""Full volume frequency analysis for SIREN and interpolation reconstructions.
...
Analysis includes all voxels in the volume (full volume analysis).
...
Usage:
    python frequency_analysis_full_volume.py
...
Outputs:
    - full_volume_frequency_analysis.pdf
"""
import sys
import os
...
def calc_kurtosis(data):
    """Calculate kurtosis with zero-division protection."""
    if np.std(data) == 0:
        return 0
    return np.mean((data - np.mean(data))**4) / np.std(data)**4


def main():
    """Main execution function for full volume frequency analysis."""
    ...
    print(f"Found {len(nodule_ids)} nodules to analyze (Full Volume Analysis)\n")
    ...
    for nodule in nodule_ids:
        ...
        # We load mask just to ensure file consistency, but don't use it for filtering
        ...
        # USE FULL VOLUMES (Flattened)
        vol_flat = vol.flatten()
        recon_interp_flat = recon_interpolation.flatten()
        recon_siren_flat = recon_SIREN.flatten()
        
        # Calculate statistics
        gt_kurt = calc_kurtosis(vol_flat)
        interp_kurt = calc_kurtosis(recon_interp_flat)
        siren_kurt = calc_kurtosis(recon_siren_flat)
        
        # Store results
        results['nodule_id'].append(nodule)
        results['gt_mean'].append(np.mean(vol_flat))
        results['gt_std'].append(np.std(vol_flat))
        results['gt_kurtosis'].append(gt_kurt)
        results['interp_mean'].append(np.mean(recon_interp_flat))
        results['interp_std'].append(np.std(recon_interp_flat))
        results['interp_kurtosis'].append(interp_kurt)
        results['siren_mean'].append(np.mean(recon_siren_flat))
        results['siren_std'].append(np.std(recon_siren_flat))
        results['siren_kurtosis'].append(siren_kurt)
        results['interp_std_ratio'].append(np.std(recon_interp_flat) / np.std(vol_flat))
        results['siren_std_ratio'].append(np.std(recon_siren_flat) / np.std(vol_flat))
    ...
    print("=" * 80)
    print("AGGREGATE STATISTICS (FULL VOLUME ANALYSIS)")
    print("=" * 80)
    ...
    # ============================================================================
    plt.tight_layout()
    file_name = 'full_volume_frequency_analysis.pdf'
